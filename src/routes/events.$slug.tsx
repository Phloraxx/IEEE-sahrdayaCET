...
import { getWieEventArtwork, WIE_SOCIAL_IMAGE_PATH } from "@/lib/wie-media";
import {
  fetchEventBySlug,
...
export const meta = ({ data }: { data?: SerializableEvent }) => {
  ...
  const artwork =
    data.society?.slug === "wie"
      ? getWieEventArtwork(data.slug, data.bannerUrl)
      : data.bannerUrl
        ? { src: data.bannerUrl, fit: "cover" as const }
        : null;
  const image = artwork
    ? artwork.src.startsWith("http")
      ? artwork.src
      : `${APP_URL}${artwork.src}`
    : data.society?.slug === "wie"
      ? `${APP_URL}${WIE_SOCIAL_IMAGE_PATH}`
      : `${APP_URL}/web.png`;
  return [
...
export default function EventDetailPage() {
  ...
  const eventArtwork =
    event.society?.slug === "wie"
      ? getWieEventArtwork(event.slug, event.bannerUrl)
      : event.bannerUrl
        ? { src: event.bannerUrl, fit: "cover" as const }
        : null;
  const eventImageUrl = eventArtwork
    ? eventArtwork.src.startsWith("http")
      ? eventArtwork.src
      : `${APP_URL}${eventArtwork.src}`
    : event.society?.slug === "wie"
      ? `${APP_URL}${WIE_SOCIAL_IMAGE_PATH}`
      : `${APP_URL}/web.png`;
  const hasCapacity =
            ...
            <div className="mt-8 grid gap-4 border-y border-slate-200 py-6 sm:grid-cols-2">
              ...
              <div className="flex gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">Kodakara, Thrissur</p>
                </div>
              </div>
            </div>
