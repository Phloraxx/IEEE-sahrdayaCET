...
export function EventWorkflowPanel({ event, permissions, pending, onAction }: {
  ...
  return <Card className="overflow-hidden"><CardContent className="p-6">
    ...
    <div className="mt-5 flex flex-wrap gap-2 border-t border-border pt-5">
      {status === "draft" && permissions["events.publish"] && <Button size="sm" className="gap-2" disabled={pending} onClick={() => onAction("publish")}><CheckCircle2 className="h-4 w-4" />Publish</Button>}
      ...
      {status === "published" && permissions["events.complete"] && <Button size="sm" variant="outline" disabled={pending} onClick={() => onAction("complete")}>Complete event</Button>}
      {status === "published" && permissions["events.publish"] && <Button size="sm" variant="outline" className="gap-2" disabled={pending} onClick={() => onAction("unpublish")}><RotateCcw className="h-4 w-4" />Return to draft</Button>}
