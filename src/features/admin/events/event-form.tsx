...
import { CalendarDays, Check, CircleDollarSign, Eye, ExternalLink, Loader2, MapPin, MessageCircle, TicketCheck, Users, WalletCards, X } from "lucide-react";
...
import { EventRequirementsEditor } from "@/features/admin/events/event-requirements-editor";
import { MAX_ATTENDEE_NOTE_LENGTH, memberPrice, normalizeEventRequirements, validateEventRequirements } from "@/lib/event-requirements";
...
interface EventFormState {
  ...
  externalLink: string; tags: string; externalFormUrl: string;
}
...
const EMPTY_STATE: EventFormState = {
  ...
  contactEmail: "", contactPhone: "", externalLink: "", tags: "", externalFormUrl: "",
};
...
export function EventForm({ mode, eventId, initialSocietyId, allowSocietyTransfer = false }: EventFormProps) {
  ...
  useEffect(() => {
    ...
    const hydrated: EventFormState = {
      ...
      externalLink: String(e.externalLink ?? ""), tags: String(e.tags ?? ""), externalFormUrl: String(e.externalFormUrl ?? ""),
    };
  ...
  const existingRecord = existing?.event ?? null;
  ...
  const validate = () => {
    ...
    if (form.registrationMode === "internal") {
      ...
      const requirementsError = validateEventRequirements(form.requirements);
      if (requirementsError) return ["registration", requirementsError] as const;
      if (form.attendeeNote.trim().length > MAX_ATTENDEE_NOTE_LENGTH) return ["registration", `Attendee note must be ${MAX_ATTENDEE_NOTE_LENGTH} characters or fewer.`] as const;
      ...
      const memberDiscount = Number(form.ieeeMemberDiscountPercent);
  ...
  const buildPayload = () => { const effectivePrice = form.registrationMode === "internal" ? Number(form.price) || 0 : 0; return ({ title: form.title.trim(), description: form.description, venue: form.attendanceMode === "online" ? "" : form.venue.trim(), timezone: form.timezone || "Asia/Kolkata", attendanceMode: form.attendanceMode, locationAddress: form.attendanceMode === "online" ? "" : form.locationAddress.trim(), price: effectivePrice,
    ...
    contactEmail: form.contactEmail.trim(), contactPhone: form.contactPhone.trim(), externalLink: form.externalLink.trim(), tags: form.tags.trim(), externalFormUrl: form.externalFormUrl.trim(),
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (isEdit && privateDirty) {
        await saveAdminEventPrivateDetails(savedEventId, form.attendanceMode === "onsite" ? { ...privateDetails, virtualJoinUrl: "", joinInstructions: "" } : privateDetails);
      }
      const savedForm: EventFormState = { ...form, venue: form.attendanceMode === "online" ? "" : form.venue, locationAddress: form.attendanceMode === "online" ? "" : form.locationAddress, price: form.registrationMode === "internal" ? String(Number(form.price) || 0) : "0", requirements: normalizeEventRequirements(form.requirements), attendeeNote: form.attendeeNote.trim(), registrationOpen: form.registrationMode !== "closed" && form.registrationOpen };
      ...
      if (!isEdit) { toast.success("Draft created. Finish the setup, then publish it from the event workspace."); navigate(`/admin/events/${savedEventId}/edit?section=details`, { replace: true }); }
      else toast.success(privateDirty && !setupChanged ? "Private attendee access saved" : "Event settings saved");
    ...
    <main className="min-w-0"><div className="rounded-2xl border border-border bg-card p-6 sm:p-8">
      ...
      {section === "registration" && <div className="space-y-7"><SectionTitle eyebrow="02 · Registration" title="How will people join?" description="Method and availability are separate. Pausing registration never deletes its setup." />
        ...
        {form.registrationMode === "internal" && <div className="space-y-7"><div className="grid gap-5 rounded-xl border border-border p-5 lg:grid-cols-2">
          ...
          <label className="flex items-center justify-between gap-4 rounded-lg border border-border px-4 py-3"><span><span className="block text-sm font-medium">IEEE membership</span><span className="mt-0.5 block text-xs text-muted-foreground">Ask attendee membership status and Membership ID.</span></span><input type="checkbox" checked={form.collectIeeeMember} disabled={Number(form.ieeeMemberDiscountPercent) > 0} onChange={(e) => patch("collectIeeeMember", e.target.checked)} /></label></div></div>
          <EventAudienceEditor
          ...
          />
          <EventRequirementsEditor requirements={form.requirements} attendeeNote={form.attendeeNote} onRequirementsChange={(value) => patch("requirements", value)} onAttendeeNoteChange={(value) => patch("attendeeNote", value)} />
      ...
      {section === "communication" && <div className="space-y-7"><SectionTitle eyebrow="04 · Communication" title="Give attendees the right ways to reach you" description="Keep contact information and public links separate from workflow status." />
        <div className="grid gap-6 md:grid-cols-2"><div className="grid gap-2"><Label htmlFor="contact-email">Contact email</Label><Input id="contact-email" type="email" value={form.contactEmail} onChange={(e) => patch("contactEmail", e.target.value)} placeholder="ieee@sahrdaya.ac.in" /></div><div className="grid gap-2"><Label htmlFor="contact-phone">Contact phone</Label><Input id="contact-phone" type="tel" value={form.contactPhone} onChange={(e) => patch("contactPhone", e.target.value)} placeholder="+91 …" /></div><div className="grid gap-2 md:col-span-2"><Label htmlFor="external-link">Public supporting link</Label><Input id="external-link" type="url" value={form.externalLink} onChange={(e) => patch("externalLink", e.target.value)} placeholder="https://..." /><p className="text-xs text-muted-foreground">Shown publicly from the event page. Never put a private meeting URL or attendee credentials here.</p></div></div>
        <div className="rounded-xl border border-primary/20 bg-primary/[0.03] p-5"><div><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-primary">Private attendee access</p><p className="mt-2 text-sm leading-6 text-muted-foreground">Only confirmed attendees can retrieve these links. They are excluded from public event data, search metadata and anonymous ticket lookup.</p></div><div className="mt-5 grid gap-5"><div className="grid gap-2"><Label htmlFor="private-whatsapp-url">Attendee WhatsApp group</Label><Input id="private-whatsapp-url" type="url" value={privateDetails.whatsappGroupUrl} onChange={(e) => { setDirty(true); setPrivateDirty(true); setPrivateDetails((current) => ({ ...current, whatsappGroupUrl: e.target.value })); }} placeholder="https://chat.whatsapp.com/..." /><p className="text-xs text-muted-foreground">Available to confirmed attendees for onsite, online and hybrid events.</p></div>{(form.attendanceMode === "online" || form.attendanceMode === "hybrid") && <><div className="grid gap-2"><Label htmlFor="private-join-url">Meeting / join URL</Label><Input id="private-join-url" type="url" value={privateDetails.virtualJoinUrl} onChange={(e) => { setDirty(true); setPrivateDirty(true); setPrivateDetails((current) => ({ ...current, virtualJoinUrl: e.target.value })); }} placeholder="https://meet.google.com/..." /></div><div className="grid gap-2"><Label htmlFor="private-join-instructions">Joining instructions</Label><Textarea id="private-join-instructions" rows={4} value={privateDetails.joinInstructions} onChange={(e) => { setDirty(true); setPrivateDirty(true); setPrivateDetails((current) => ({ ...current, joinInstructions: e.target.value })); }} placeholder="Optional attendee-only instructions" maxLength={4000} /></div></>}</div></div>
      ...
      {section === "preview" && <div className="space-y-7"><SectionTitle eyebrow="05 · Review" title="See the setup before you publish it" description="This is an organizer preview. Publishing and closeout actions stay in the event workspace." />
        <div className="overflow-hidden rounded-2xl border border-border bg-background">{bannerUrl && <img src={bannerUrl} alt="" className="aspect-[16/6] w-full object-cover" />}<div className="p-6 sm:p-8"><div className="flex flex-wrap gap-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground"><span>{form.status}</span><span>·</span><span>{form.registrationMode === "internal" ? "IEEE registration" : form.registrationMode === "external" ? "External registration" : "No registration"}</span></div><h3 className="mt-3 text-3xl font-semibold tracking-tight">{form.title || "Untitled event"}</h3><div className="mt-5 grid gap-4 text-sm sm:grid-cols-3"><div><p className="text-xs text-muted-foreground">When</p><p className="mt-1 font-medium">{form.date ? formatEventDateTime((form.timeTbc ? fromAppDateOnly(form.date) : fromAppDateTimeLocal(form.date)) || "", form.timeTbc) : "Not set"}</p></div><div><p className="text-xs text-muted-foreground">Where</p><p className="mt-1 font-medium">{form.attendanceMode === "online" ? "Online" : form.venue || "Not set"}</p><p className="mt-1 text-xs capitalize text-muted-foreground">{form.attendanceMode.replace("onsite", "on site")}</p></div><div><p className="text-xs text-muted-foreground">Entry</p><p className="mt-1 font-medium">{form.registrationMode === "internal" && Number(form.price) > 0 ? `₹${form.price}` : "Free / external"}</p>{form.registrationMode === "internal" && Number(form.price) > 0 && Number(form.ieeeMemberDiscountPercent) > 0 && <p className="mt-1 text-xs text-primary">IEEE members: ₹{memberPrice(Number(form.price), Number(form.ieeeMemberDiscountPercent))} · {form.ieeeMemberDiscountPercent}% off</p>}</div></div></div></div>
        {form.registrationMode === "internal" && <div className="rounded-2xl border border-border p-6 sm:p-8"><div className="border-b border-border pb-5"><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-primary">Audience</p><p className="mt-2 text-sm font-semibold">{form.eligibleSemesters.length || form.eligibleProgrammes.length ? "Restricted registration" : "Open to all programmes and semesters"}</p><p className="mt-1 text-xs leading-5 text-muted-foreground">{form.eligibleSemesters.length ? `Semesters: ${form.eligibleSemesters.join(", ")}` : "All semesters"} · {form.eligibleProgrammes.length ? `${form.eligibleProgrammes.length} programme${form.eligibleProgrammes.length === 1 ? "" : "s"}` : "All programmes"}</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="rounded-lg border border-border px-3 py-2"><p className="text-[10px] uppercase tracking-wide text-muted-foreground">Before you attend</p><p className="mt-1 text-sm font-semibold">{normalizeEventRequirements(form.requirements).length} requirement{normalizeEventRequirements(form.requirements).length === 1 ? "" : "s"}</p></div><div className="rounded-lg border border-border px-3 py-2"><p className="text-[10px] uppercase tracking-wide text-muted-foreground">Attendee note</p><p className="mt-1 text-sm font-semibold">{form.attendeeNote.trim() ? "Included" : "Not set"}</p></div></div></div><div className="mt-6 flex items-center justify-between gap-4"><div><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-primary">Attendee form</p><h3 className="mt-2 text-xl font-semibold">What participants will be asked</h3></div><span className="rounded-full border border-border px-2.5 py-1 text-xs text-muted-foreground">{customFields.length} custom</span></div><div className="mt-6 grid gap-5 sm:grid-cols-2">{STANDARD_FIELDS.map(([name]) => <div key={name} className="space-y-2"><div className="text-xs font-semibold">{name}</div><div className="h-10 rounded-lg border border-input bg-muted/20" /></div>)}</div>{customFields.length > 0 && <div className="mt-7 space-y-5 border-t border-border pt-7">{customFields.map((field) => <QuestionPreview key={field.id} field={field} />)}</div>}</div>}
        <div className="rounded-xl border border-border p-4"><p className="text-xs text-muted-foreground">Attendee communication</p><p className="mt-2 text-sm font-semibold">WhatsApp {privateDetails.whatsappGroupUrl.trim() ? "configured" : "not configured"} · Online access {(privateDetails.virtualJoinUrl.trim() || privateDetails.joinInstructions.trim()) ? "configured" : "not configured"}</p></div>
        <div className="grid gap-3 md:grid-cols-3"><div className="rounded-xl border border-border p-4"><p className="text-xs text-muted-foreground">Lifecycle status</p><p className="mt-2 font-semibold capitalize">{form.status}</p></div><div className="rounded-xl border border-border p-4"><p className="text-xs text-muted-foreground">Registration</p><p className="mt-2 font-semibold">{form.registrationMode === "closed" ? "No registration" : form.registrationOpen ? "Open or scheduled" : "Paused"}</p></div><div className="rounded-xl border border-border p-4"><p className="text-xs text-muted-foreground">Next step</p><p className="mt-2 font-semibold">{form.status === "published" ? "Operate the event" : form.status === "completed" ? "Resolve closeout" : "Save, then publish when ready"}</p></div></div>
     ...
     <div className="fixed inset-x-0 bottom-0 z-30 border-t border-border bg-background/95 lg:left-64 backdrop-blur supports-[backdrop-filter]:bg-background/85"><div className="mx-auto flex max-w-[1600px] items-center justify-between gap-4 px-4 py-3 sm:px-6"><div className="min-w-0"><p className="truncate text-sm font-medium">{dirty ? "Unsaved changes" : "All changes saved"}</p><p className="hidden text-xs text-muted-foreground sm:block">Publishing and closeout actions stay in the event workspace.</p></div><div className="flex shrink-0 gap-2"><Button type="button" variant="outline" onClick={leaveEditor}>Back to workspace</Button><Button type="submit" disabled={submitting || !dirty} className="gap-2">{submitting && <Loader2 className="h-4 w-4 animate-spin" />}Save changes</Button></div></div></div>
  </form>;
}
