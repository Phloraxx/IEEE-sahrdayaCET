...
interface SocietyActivitySignal {
  eventCount: number;
...
const DEFAULT_ACCENT: Accent = {
  ...
  line: "bg-ieee-blue",
  ghost: "text-ieee-blue/[0.055]",
};
const SOCIETY_ACCENTS: Record<string, Accent> = {
  cas: { text: "group-hover:text-emerald-700", wash: "group-hover:bg-emerald-50/30", line: "bg-emerald-600", ghost: "text-emerald-500/[0.065]" },
  css: { text: "group-hover:text-sky-700", wash: "group-hover:bg-sky-50/30", line: "bg-sky-600", ghost: "text-sky-500/[0.06]" },
  cs: { text: "group-hover:text-blue-700", wash: "group-hover:bg-blue-50/30", line: "bg-blue-600", ghost: "text-blue-500/[0.06]" },
  edsoc: { text: "group-hover:text-indigo-700", wash: "group-hover:bg-indigo-50/30", line: "bg-indigo-600", ghost: "text-indigo-500/[0.06]" },
  embs: { text: "group-hover:text-fuchsia-700", wash: "group-hover:bg-fuchsia-50/30", line: "bg-fuchsia-600", ghost: "text-fuchsia-500/[0.06]" },
  ies: { text: "group-hover:text-orange-700", wash: "group-hover:bg-orange-50/30", line: "bg-orange-600", ghost: "text-orange-500/[0.06]" },
  ias: { text: "group-hover:text-lime-700", wash: "group-hover:bg-lime-50/30", line: "bg-lime-600", ghost: "text-lime-500/[0.065]" },
  npss: { text: "group-hover:text-rose-700", wash: "group-hover:bg-rose-50/30", line: "bg-rose-600", ghost: "text-rose-500/[0.06]" },
  pes: { text: "group-hover:text-emerald-700", wash: "group-hover:bg-emerald-50/30", line: "bg-emerald-600", ghost: "text-emerald-500/[0.065]" },
  ras: { text: "group-hover:text-red-700", wash: "group-hover:bg-red-50/30", line: "bg-red-600", ghost: "text-red-500/[0.06]" },
  sight: { text: "group-hover:text-orange-700", wash: "group-hover:bg-orange-50/30", line: "bg-orange-600", ghost: "text-orange-500/[0.06]" },
  sps: { text: "group-hover:text-teal-700", wash: "group-hover:bg-teal-50/30", line: "bg-teal-600", ghost: "text-teal-500/[0.06]" },
  wie: { text: "group-hover:text-purple-700", wash: "group-hover:bg-purple-50/30", line: "bg-purple-600", ghost: "text-purple-500/[0.06]" },
};
...
export default function SocietiesClient({ societies, activityBySociety }: SocietiesClientProps) {
    ...
    <div className="relative min-h-screen w-full bg-white font-sans text-slate-950 selection:bg-ieee-blue/15">
      ...
      <main className="relative z-10 px-5 pb-24 pt-28 sm:px-6 sm:pt-32">
        <div className="mx-auto max-w-7xl">
          ...
          <section className="hidden py-5 md:block" aria-label="Society network status">
            ...
            <div className="relative mt-4 py-2">
              ...
              />
              <div aria-hidden="true" className="relative grid" style={{ gridTemplateColumns: `repeat(${societies.length}, minmax(0, 1fr))` }}>
                {societies.map((society) => {
                  const active = society.id === activeSocietyId;
                  const accent = societyAccent(society.slug);
                  const hasActivity = Boolean(activityBySociety[society.id]?.nextEvent);
                  return (
                    <div key={society.id} className="flex justify-center">
                      <span className={`relative block h-2.5 w-2.5 rounded-full border transition-all duration-300 ${active ? `scale-[1.45] border-white ring-1 ring-slate-300 ${accent.line}` : hasActivity ? `border-white ring-1 ring-slate-300 ${accent.line}` : "border-slate-300 bg-white"}`}>
                        {hasActivity && !active && <span className={`absolute inset-[-3px] rounded-full opacity-20 ${accent.line}`} />}
                      </span>
                    </div>
                  );
                })}
          ...
          <section className="sticky top-20 z-20 border-y border-slate-200 bg-white/92 py-3 backdrop-blur-xl">
              ...
              <motion.div layout className="grid gap-4 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
                {filtered.map((society, index) => (
                  <motion.div key={society.id} layout initial={reduceMotion ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, delay: reduceMotion ? 0 : Math.min(index * 0.025, 0.2) }}>
                    <GridCard society={society} index={societies.indexOf(society)} total={societies.length} activity={activityBySociety[society.id]} onActivate={setActiveSocietyId} />
                  </motion.div>
                ))}
              </motion.div>
            ) : (
              <motion.div layout>
                {filtered.map((society) => (
                  <motion.div key={society.id} layout initial={reduceMotion ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }}>
                    <ListRow society={society} index={societies.indexOf(society)} activity={activityBySociety[society.id]} onActivate={setActiveSocietyId} />
                  </motion.div>
                ))}
              </motion.div>
            )}
