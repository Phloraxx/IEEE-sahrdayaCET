...
interface AdminTopbarProps { onOpenSidebar: (e: React.MouseEvent<HTMLElement>) => void; sidebarOpen: boolean; isNavigating: boolean; }
const LABELS: [string, string][] = [
  ...
  ["/admin/users", "Users & Access"], ["/admin/execom", "Execom"], ["/admin/blogs", "Blogs"], ["/admin/FIFA", "FIFA"],
];

export function AdminTopbar({ onOpenSidebar, sidebarOpen, isNavigating }: AdminTopbarProps) {
  const location = useLocation();
  const navigate = useNavigate();
  const [query, setQuery] = useState("");
  const [environment, setEnvironment] = useState("ADMIN");
  useEffect(() => {
    const host = window.location.hostname;
    setEnvironment(host.startsWith("staging.") ? "STAGING" : host === "ieeesahrdaya.com" || host === "www.ieeesahrdaya.com" ? "PRODUCTION" : "LOCAL");
  }, []);
  const page = useMemo(() => LABELS.find(([prefix]) => location.pathname === prefix || location.pathname.startsWith(`${prefix}/`))?.[1] ?? "Admin", [location.pathname]);
  ...
  return <>
    <header className="sticky top-0 z-30 flex h-14 items-center gap-3 border-b border-border bg-background/95 px-4 backdrop-blur supports-[backdrop-filter]:bg-background/80 lg:h-16 lg:px-6 vh-safe-top">
      ...
      <form onSubmit={search} className="ml-auto hidden w-full max-w-sm md:block"><div className="relative"><Search className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" /><input value={query} onChange={(e) => setQuery(e.target.value)} className="h-9 w-full rounded-md border border-border bg-muted/25 pl-9 pr-3 text-sm outline-none transition-colors placeholder:text-muted-foreground focus:border-foreground/30 focus:bg-background" placeholder="Search attendee, email, phone or ticket…" aria-label="Search registrations" /></div></form>
      <Link to="/admin/events/new" className="hidden h-9 items-center gap-1.5 rounded-md bg-primary px-3 text-xs font-medium text-primary-foreground hover:opacity-90 sm:inline-flex"><Plus className="h-3.5 w-3.5" />New event</Link>
    </header>
