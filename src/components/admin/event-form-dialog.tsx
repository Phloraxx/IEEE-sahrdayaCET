...
export function EventFormDialog({
  ...
  // Existing event payload — only when editing
  const { data: existing } = useQuery<{ event: Record<string, unknown> }>({
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      // Drop undefined keys so PB doesn't complain about unknown columns
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!form.society) {
      setSubmitError("Society is required");
      return;
    }
    mutation.mutate();
  };
