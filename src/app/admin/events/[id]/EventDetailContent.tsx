...
export async function EventDetailContent({ eventId }: { eventId: string }) {
  ...
  try {
    const event = await pb
      .collection("events")
      .getOne(eventId, { expand: "society" });
    const expand = (event as Record<string, unknown>).expand as
    ...
    if (!societyName) {
      ...
      if (societyId) {
        try {
          const societyRecord = await pb
            .collection("societies")
            .getOne(societyId, { fields: "id,name" });
          societyName = (societyRecord as Record<string, unknown>)
            .name as string;
        } catch {
          // Non-fatal
        }
    ...
    try {
      const registrations = await pb
        ...
        .getList(1, 50, {
          ...
          fields:
            "id,userName,userEmail,userPhone,registrationStatus,paymentStatus,checkedIn,checkedInAt,ticketId,paymentTicketId,created,amount",
        });
    ...
    return (
      <EventDetailClient
        event={{
          ...
          status:
            ((event as Record<string, unknown>).status as string) || "draft",
          registrationOpen: !!(event as Record<string, unknown>)
            .registrationOpen,
          maxCapacity:
            Number((event as Record<string, unknown>).maxCapacity) || 0,
          registeredCount:
            Number((event as Record<string, unknown>).registeredCount) || 0,
          checkedInCount:
            Number((event as Record<string, unknown>).checkedInCount) || 0,
          isPaid: Number((event as Record<string, unknown>).price) > 0,
          societyName: societyName,
          registrationDeadline: (event as Record<string, unknown>)
            .registrationDeadline as string,
          contactEmail: (event as Record<string, unknown>)
            .contactEmail as string,
          contactPhone: (event as Record<string, unknown>)
