...
export async function OverviewContent() {
  ...
  const pb = createPB(
    `${PB_AUTH_COOKIE}=${cookieStore.get(PB_AUTH_COOKIE)?.value}`,
  );
  ...
  try {
    ...
    const recentRes = await pb.collection("registrations").getList(1, 8, {
      ...
      expand: "event",
      fields:
        "id,userName,userEmail,registrationStatus,paymentStatus,checkedIn,registrationDate,expand.event.id,expand.event.title",
    });
    ...
    const dailyRegistrations = Object.entries(dailyMap).map(([date, c]) => ({
      date,
      count: c,
    }));
