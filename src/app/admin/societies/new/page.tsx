...
export default function NewSocietyPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (form.defaultWhatsappLink) fd.append('defaultWhatsappLink', form.defaultWhatsappLink)
      ...
      router.push(`/admin/societies/${data.society.id}`)
    } catch (err) {
    ...
    <div className="space-y-6 max-w-2xl">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Add a new IEEE society</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          <button type="submit" disabled={saving}
            ...
            {saving ? <><Loader2 className="size-4 mr-2 animate-spin" /> Creating...</> : 'Create Society'}
          </button>
