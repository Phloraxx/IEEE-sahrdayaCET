...
export default function ExecomPage() {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true)
    try {
      const res = await fetch(`/api/admin/execom/${deleteId}`, { method: 'DELETE' })
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Manage executive committee members.</p>
          </div>
        </div>
        <Card>
    ...
    <div className="space-y-6">
        ...
        <CardContent className="p-0">
          ...
          {filtered.length === 0 ? (
            <div className="p-12 text-center text-sm text-muted-foreground">
              {searchQuery ? 'No members match your search.' : 'No execom members yet.'}
            </div>
          ) : (
            <>
                    ...
                    {paged.map((m) => (
                      <TableRow key={m.id}>
                        <TableCell>
                          ...
                          <div className="text-xs text-muted-foreground">{m.department || ''} {m.batch ? `· ${m.batch}` : ''}</div>
                        </TableCell>
                        <TableCell className="hidden sm:table-cell text-muted-foreground">{m.position}</TableCell>
                        <TableCell className="hidden md:table-cell text-muted-foreground">{m.department || '—'}</TableCell>
                        <TableCell className="hidden md:table-cell text-muted-foreground">
                          {m.expand?.society?.name || '—'}
                        </TableCell>
                        <TableCell className="text-right">
                          <div className="flex items-center justify-end gap-1">
                            <Button variant="ghost" size="icon" className="size-8" onClick={() => router.push(`/admin/execom/${m.id}/edit`)}>
                              <Pencil className="size-3.5" />
                            </Button>
                            <Button variant="ghost" size="icon" className="size-8" onClick={() => setDeleteId(m.id)}>
      ...
      <Dialog open={!!deleteId} onOpenChange={(open) => { if (!open) setDeleteId(null) }}>
        <DialogContent>
          ...
          <p className="text-sm text-muted-foreground">Are you sure you want to delete this execom member?</p>
          <DialogFooter>
