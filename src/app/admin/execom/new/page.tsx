...
export default function NewExecomPage() {
  ...
  useEffect(() => {
    fetch('/api/admin/societies')
      .then((r) => r.json())
      .then((data) => {
        setSocieties((data.societies || []).map((s: { id: string; name: string }) => ({ id: s.id, name: s.name })))
      })
      .catch(() => {})
  }, [])
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Add Execom Member</h1>
          <p className="text-sm text-muted-foreground mt-1">Add a new executive committee member</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="rounded-xl border bg-card p-6 space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <input value={form.batch} onChange={update('batch')} placeholder="e.g. 2024-28"
                className="w-full rounded-lg border border-input bg-white px-3 py-2 text-sm outline-none focus:border-ring focus:ring-1 focus:ring-ring/50" />
            </div>
          </div>
        ...
        <div className="flex items-center gap-3">
          <button type="submit" disabled={saving}
            ...
            {saving ? <><Loader2 className="size-4 mr-2 animate-spin" /> Creating...</> : 'Create Member'}
          </button>
