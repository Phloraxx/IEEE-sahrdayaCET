...
export default function EditExecomPage(props: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    props.params.then(({ id }) => {
      Promise.all([
        fetch('/api/admin/societies').then((r) => r.json()),
        fetch(`/api/admin/execom/${id}`).then((r) => r.json()),
      ])
        .then(([socData, memberData]) => {
          setSocieties((socData.societies || []).map((s: { id: string; name: string }) => ({ id: s.id, name: s.name })))
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div className="flex items-center gap-4">
        <Link href="/admin/execom" className="inline-flex items-center justify-center rounded-lg border border-border p-2 text-muted-foreground hover:bg-muted transition-colors">
          <ArrowLeft className="size-4" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Edit Execom Member</h1>
          <p className="text-sm text-muted-foreground mt-1">{form.name || 'Loading...'}</p>
        </div>
      </div>
        ...
        <div className="rounded-xl border bg-card p-6 space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <input required value={form.position} onChange={update('position')}
                className="w-full rounded-lg border border-input bg-white px-3 py-2 text-sm outline-none focus:border-ring focus:ring-1 focus:ring-ring/50" />
            </div>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <input value={form.batch} onChange={update('batch')}
                className="w-full rounded-lg border border-input bg-white px-3 py-2 text-sm outline-none focus:border-ring focus:ring-1 focus:ring-ring/50" />
            </div>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <input value={form.section} onChange={update('section')}
                className="w-full rounded-lg border border-input bg-white px-3 py-2 text-sm outline-none focus:border-ring focus:ring-1 focus:ring-ring/50" />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">Section ID</label>
              <input value={form.sectionId} onChange={update('sectionId')}
                className="w-full rounded-lg border border-input bg-white px-3 py-2 text-sm outline-none focus:border-ring focus:ring-1 focus:ring-ring/50" />
            </div>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <select value={form.society} onChange={update('society')}
                className="w-full rounded-lg border border-input bg-white px-3 py-2 text-sm outline-none focus:border-ring focus:ring-1 focus:ring-ring/50">
            ...
            <div className="space-y-2">
              ...
              <input type="number" min="0" value={form.order} onChange={update('order')}
                className="w-full rounded-lg border border-input bg-white px-3 py-2 text-sm outline-none focus:border-ring focus:ring-1 focus:ring-ring/50" />
            </div>
          </div>
        </div>
