...
export async function GET(
  ...
  try {
    ...
    const society = await pb.collection('societies')
    ...
    const [events, members] = await Promise.all([
      ...
      pb.collection('execom')
        .getList(1, 50, {
          ...
          sort: 'order',
          skipTotal: true,
          fields: 'id,order,name,department,batch,position,photo,linkedin,instagram',
        })
        .then((res) => res.items)
        .catch(() => []),
    ])
    ...
    return Response.json({
      society: {
        ...
        name: society.name,
        slug: society.slug,
        bio: (society as Record<string, unknown>).bio as string || '',
        logoUrl: (society as Record<string, unknown>).logo
          ? buildFileUrl('societies', society.id, (society as Record<string, unknown>).logo as string)
          : '',
        bannerUrl: (society as Record<string, unknown>).banner
          ? buildFileUrl('societies', society.id, (society as Record<string, unknown>).banner as string)
          : '',
      },
      events: mappedEvents,
