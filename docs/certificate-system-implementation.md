...
- existing production SMTP network reachability and STARTTLS succeeded. The sender domain SPF authorizes Google. DMARC is currently absent, and a common Google Workspace DKIM selector was not published, so aligned DKIM remains unproven and must be confirmed by the domain owner before real certificate bulk delivery;
