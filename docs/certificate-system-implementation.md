...
## Phase 15 — production-readiness rehearsal

Completed as a read-only/reversible rehearsal only. Production `main`, the production PocketBase volume, certificate issuance, and outbound mail were not changed.

- production remained on `main @ 6787b614086b3343271e2d04afdc82169ae76d31`; both canonical Dokploy containers were healthy and the checkout provenance matched the production project;
- current production `/`, `/events`, `/societies`, `/blog`, `/full-execom`, FIFA routes, `/healthz`, and `/api/health` returned 200 with a browser-like request profile; public `/_/` returned 404 and Google remained the configured OAuth provider;
- the exact staging-accepted runtime candidate `3f9ec8c1a1e6e581deea9ca289edf56f5fb82904` was tested with `CHECK_RUNTIME=0` against the actual rendered production `.env`. `DEPLOY_ENV`, `SITE_URL`, SMTP host/port/sender, and SMTP auth-pair checks passed. Release failed closed only because `CERTIFICATE_RENDER_CAPABILITY_KEY`, explicit `CERTIFICATE_MAIL_PROVIDER=smtp`, and `MAIL_DELIVERY_MODE=live` are intentionally not yet configured in production;
- existing production SMTP network reachability and STARTTLS succeeded, and the SMTP login/From domains align. SPF authorizes Google. However, delivered IEEE Sahrdaya mail from both April and August 2026 shows Gmail receiver results `dkim=permerror (no key for signature)` for `d=sahrdaya.ac.in; s=google`; the matching DNS selector is absent and DMARC is also absent. DKIM/DMARC remediation by the college domain owner is therefore a concrete blocker before real certificate bulk delivery;
- current production notification history contains 257 `sent` outbox rows (128 ticket, 129 receipt), proving the existing SMTP transport has been accepting operational mail. This does not replace the deferred `[TEST / NOT VALID]` certificate inbox-delivery acceptance;
- a consistent SQLite online backup of the running production database passed `PRAGMA integrity_check=ok` with 185 applied migrations. The exact candidate PocketBase image booted that copy in an isolated non-public container and applied all 12 pending migrations successfully, yielding 197 migrations and another `integrity_check=ok`;
- every pre-existing application table preserved its row count through the rehearsal: 20/20 unchanged, including users 244→244, events 82→82, registrations 248→248, societies 15→15, blogs 5→5, execom 91→91, and notification outbox 257→257;
- the isolated container, database copies, Docker volume/image, and temporary encryption-key handoff were deleted immediately after the rehearsal; real production containers remained healthy throughout;
