...
migrate((app) => {
  ...
  if (!outbox.fields.getByName("deliveryProvider")) outbox.fields.add(new SelectField({
    name: "deliveryProvider", values: ["smtp", "resend"], maxSelect: 1,
  }))
  if (!outbox.fields.getByName("providerMessageId")) outbox.fields.add(new TextField({
    name: "providerMessageId", max: 240,
  }))
  if (!outbox.fields.getByName("providerSendSequence")) outbox.fields.add(new NumberField({
    name: "providerSendSequence", min: 0,
  }))
  if (!outbox.fields.getByName("providerMessageHeader")) outbox.fields.add(new TextField({
    name: "providerMessageHeader", max: 500,
  }))
  if (!outbox.fields.getByName("providerStatus")) outbox.fields.add(new SelectField({
    name: "providerStatus",
    values: ["accepted", "sent", "delivered", "delayed", "bounced", "failed", "suppressed", "complained"],
    maxSelect: 1,
  }))
  ...
  try {
    ...
    var outboxFields = ["deliveryProvider", "providerMessageId", "providerSendSequence", "providerMessageHeader", "providerStatus", "providerUpdatedAt", "deliveredAt", "providerError"]
    for (var i = 0; i < outboxFields.length; i++) {
      var field = outbox.fields.getByName(outboxFields[i])
