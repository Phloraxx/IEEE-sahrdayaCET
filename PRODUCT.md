...
The primary public surface (`/FIFA`, `/FIFA/leaderboard`, `/FIFA/matches`) is brand: the impression IS the product — a vibrant, game-day experience that makes students want to join. The authenticated dashboard (`/FIFA/dashboard`) and admin panel (`/admin/FIFA`) are product surfaces and follow product conventions, but the default register for the game is brand.

## Users

Sahrdaya College of Engineering & Technology students, roughly 18-22, joining a free-to-enter FIFA World Cup prediction game for fun and a sponsor voucher prize. They open it on their phones between classes, during the quarterfinals onward (June 10 start). Context: casual, social, competitive with friends, low attention span, thumb-driven. They want to (1) see the next match and current odds, (2) place a bet in under 15 seconds, (3) watch live match/pool updates and the leaderboard move, (4) check their rank. The admin is one or two IEEE student-branch members managing matches, entering results, and triggering the raffle.

## Product Purpose

A points-based match prediction game (fake points, no real money) on top of the existing IEEE Sahrdaya site, for the 2026 FIFA World Cup quarterfinals onward. Free to enter, college-email-only via Google OAuth, with a sponsor voucher prize decided by a weighted raffle at the end. Success = high participation (most active students playing every match day), responsive live match/pool updates, fair settlement, and a clean raffle draw. Not a gambling product — keep currency fake and the tone playful.
...
3. **Live is the point.** Match state and pool splits update in real time. Movement = presence = "people are playing right now."
...
- Reduced-motion alternatives for every animation, including pool-bar and match-state transitions.
