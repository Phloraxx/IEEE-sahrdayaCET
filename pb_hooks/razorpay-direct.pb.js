...
cronAdd("razorpay-refund-worker", "*/2 * * * *", function () {
  var helpers = require(__hooks + "/razorpay-direct-helpers.js")
  var state = require(__hooks + "/razorpay-payment-state.js")
  var config = helpers.getConfig()
  if (!helpers.apiConfigured(config)) return
  var refunds = []
  try {
    refunds = $app.findRecordsByFilter(
      "payment_refunds",
      "(status = {:queued} || status = {:submitted}) && attempts < 8",
      "created", 20, 0,
      { queued: "queued", submitted: "submitted" }
    )
  } catch (err) { console.log("[razorpay] refund queue scan failed:", err); return }

  for (var i = 0; i < refunds.length; i++) {
    var refundId = refunds[i].id
    var providerRefundId = refunds[i].getString("providerRefundId") || ""
    var payment = null
    try { payment = $app.findRecordById("payments", refunds[i].getString("payment")) } catch (_) {}
    if (!payment || !payment.getString("capturedPaymentId")) continue
    var capturedPaymentId = payment.getString("capturedPaymentId")
    try {
      $app.runInTransaction(function(txApp) {
        var current = txApp.findRecordById("payment_refunds", refundId)
        current.set("attempts", (current.getInt("attempts") || 0) + 1)
        current.set("lastAttemptAt", new Date().toISOString())
        txApp.saveNoValidate(current)
      })

      var response
      if (providerRefundId) {
        response = helpers.apiRequest(config, "/v1/payments/" + encodeURIComponent(capturedPaymentId) + "/refunds/" + encodeURIComponent(providerRefundId), "GET", null, {})
      } else {
        response = helpers.apiRequest(
          config,
          "/v1/payments/" + encodeURIComponent(capturedPaymentId) + "/refund",
          "POST",
          {
            amount: refunds[i].getInt("amountPaise") || 0,
            notes: {
              ieee_refund_id: refundId,
              source: refunds[i].getString("source") || "reconciliation",
            },
          },
          { "X-Refund-Idempotency": refunds[i].getString("idempotencyKey") }
        )
      }
      if (response.statusCode !== 200 && response.statusCode !== 201) throw new Error("Razorpay refund request returned " + response.statusCode)
      var applied = state.applyRefundResponse($app, refundId, response.json)
      if (!applied.ok) throw new Error(applied.error || "Refund transition failed")

      if (applied.providerStatus === "processed") {
        var paymentResponse = helpers.apiRequest(config, "/v1/payments/" + encodeURIComponent(capturedPaymentId), "GET", null, {})
        if (paymentResponse.statusCode === 200) {
          var latestPayment = $app.findRecordById("payments", applied.paymentId)
          var paymentOutcome = state.applyProviderPayment(
            $app,
            latestPayment.getString("registration"),
            paymentResponse.json,
            latestPayment.getString("providerOrderId")
          )
          if (!paymentOutcome.ok) throw new Error(paymentOutcome.error || "Refund payment reconciliation failed")
        }
      }
    } catch (err) {
      console.log("[razorpay] refund worker failed for " + refundId + ":", err)
      try {
        $app.runInTransaction(function(txApp) {
          var failed = txApp.findRecordById("payment_refunds", refundId)
          var attempts = failed.getInt("attempts") || 0
          failed.set("failureReason", String(err && err.message || err).slice(0, 4000))
          if (attempts >= 8) {
            failed.set("status", "failed")
            failed.set("failedAt", new Date().toISOString())
            try {
              var reviewPayment = txApp.findRecordById("payments", failed.getString("payment"))
              reviewPayment.set("manualReview", true)
              reviewPayment.set("reviewReason", "Razorpay refund retries were exhausted")
              txApp.saveNoValidate(reviewPayment)
            } catch (_) {}
          } else if (!failed.getString("providerRefundId")) {
            failed.set("status", "queued")
          } else {
            failed.set("status", "submitted")
          }
          txApp.saveNoValidate(failed)
        })
      } catch (_) {}
